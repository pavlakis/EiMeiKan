<?php
require_once '../vendor/autoload.php';

use EiMeiKan\Events\Events;

?>
<!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
        <title>Ei Mei Kan Dojo | Leicester Aikido Club | United Kindgom Aikikai</title>
        <meta name="description" content="Ei Mei Kan is a UKA dojo based in Leicester and teaching traditional aikido.">
        <meta name="keywords" content="Leicester, Aikido, Aikikai, Ren Shin Kan, Ei Mei Kan, Antonis, Pavlakis, sensei, Antonios Pavlakis, Terry, Mickowski, Philip Smith, UKA, hombu, self-defence, martial art, dmu, de montfort university, JAC, Joint Aikikai Council, faircharm trading estate, アドーニ" />
        <meta name="viewport" content="width=device-width">
        <link rel="stylesheet" href="css/normalize.min.css">
        <link rel="stylesheet" href="css/aikido.css?v=2">
        <script src="js/vendor/modernizr-2.6.2.min.js"></script>
    </head>
    <body>
        <div class="org">United Kingdom Aikikai</div>
        <header>
        <h1><span>栄明館</span>Ei Mei Kan Aikido</h1>
            
            <nav>
                <ul>
                    <li>
                        <a href="#welcome">Welcome</a>
                    </li>
                    <li>
                        <a href="#beginners">Beginners</a>
                    </li>
                    <li>
                        <a href="#instructors">Instructors</a>
                    </li>
                    <li>
                        <a href="#events">Events</a>
                    </li>
                    <li>
                        <a href="#resources">Resources</a>
                    </li>
                    <li>
                        <a href="#location">Location</a>
                    </li>
                </ul>
            </nav>
        </header>
        <section>
            <article id="welcome">
                <div>
                    <h2>Welcome - はじめまして</h2>
                    <p>We practice the Japanese martial art of Aikido at the <a href="#location">Faircharm Trading Estate</a> in Leicester </p>
                    <img class="large" src="img/studio_1.jpg" />
                    <p>This is a new club that started in May 2013. Classes are every <u>Friday 7pm - 8.30pm</u></p>
                    <p>
                        Our club is part of the <a href="http://www.ukaonline.org.uk">United Kingdom Aikikai</a>.
                    </p>
                    <p>
                        If you're interested in starting aikido or if you already train, you're welcome to come along and join us.
                    </p>
                    
                    <p>
                        Follow us on Twitter <a href="http://www.twitter.com/EiMeiKanDojo">@EiMeiKanDojo</a> 
                    </p>
	           <p>
			Like us on <a href="https://www.facebook.com/EiMeiKan">Facebook</a>
		   </p>
  
                </div>
            </article>
            <article id="beginners">
                <div>
                    <h2>Beginners</h2>
                    <p>
                        If you are interested to try aikido, come along on one of our classes. The first class if FREE!                      
                    </p>
                    <p>
                        Wear loose clothing like a t-shirt and jogging bottoms.
                    </p>
                    <h3>Frequently Asked Questions</h3>
                    <ul>
                        <li>
                            <p>What is aikido?</p>
                            <p>Aikido is a non-competitive Japanese martial art created in the early 1940's by Morihei Ueshiba. A good introductory text
                                can be found on the <a href="https://en.wikipedia.org/wiki/Aikido">aikido wikipedia page</a></p>
                        </li>
                        <li>
                            <p>What do I need to get started?</p>
                            <p>Just come along to one of our classes. Wear loose clothing like a t-shirt and jogging bottoms. The first class is free.</p>
                            <p>If you join the club it would be good to get a keikogi (also referred to as 'gi'). These are the same as judo or karate white uniforms. They cost
                                around &pound;20 and you can get them from any martial arts shop.                           
                            </p>
                        </li>                        
                        <li>
                            <p>How much does it cost?</p>
                            <p>Classes cost &pound;5 per lesson for adults and &pound;3 for students and unwaged.</p>
                        </li>
                        <li>
                            <p>How can I join?</p>
                            <p>
                                After your first class, if you do decide to join our club, you must join the UKA. Membership costs &pound;32 per year (&pound;25 for students/unwaged) and also covers your insurance (indemnity insurance required by all martial artists).
                                Complete this <a href="docs/UKA_Membership_Form_2011_EiMeiKan.pdf">membership form</a> 
                                and give it to the instructor together with your payment. If you're paying by cheque, it will need to be payable to "Renshinkan Affiliations".
                            </p>
                        </li>
			<li>
			    <p>What's in a name?</p>
			    <p>Aikido clubs normally receive their name by their instructor. In our case, we were given our dojo name: Ei Mei Kan (written: 栄明館 in kanji) by <a href="https://www.facebook.com/PhilipSmith6thDanShihan">Philip Smith shihan</a> from <a href="http://www.aikidodudley.org.uk/">Ren Shin Kan</a> dojo.</p>
			    <p>In Japanese, words may be pronounced the same (and written the same way in English) but according to their kanji they have a different meaning. Ei Mei Kan means House of Central Light, and with the actual kanji loosely translates as house/school of glorious honor, with the kanji 栄明 - to mean - glorious honor, and 館 - house/school. </p>
			</li>                      
                    </ul>
                        

                </div>
            </article>
            
            <article id="instructors">
                <div>
                    <h2>Instructors</h2>
                    <img src="img/Antonis_Pavlakis.jpg" />
                    <p>Antonios Pavlakis 1st dan (shodan so-Hombu) has been studying aikido since 1997 and has a Sports England teaching qualification (CL1) through the Joint Aikikai Council (JAC).</p>
                    
                    <p>Antonis started Aikido at De Montfort University Aikido Club under the instruction of Dave Lenton sensei. In 1999, Dave Lenton sensei (4th dan) passed the club to Terry Mickowski sensei (4th dan) with whom he has been training ever since.</p>
                    <p>
                        In 2009, when he received his teaching qualifications be became an assistant instructor at the DMU Aikido club.
                    </p>
                    <p>He often travels abroad to attend aikido classes and over the years has practiced with 
                    J. C. Aegerter shihan, N. Tamura shihan, C. Tissier shihan, Ian Grubb shidoin,  Y. Kobayashi shihan, as well as the present Doshu.</p>

                    <p>He gets regular tuition from Philip Smith shihan (6th dan) from Ren Shin Kan dojo and other aikikai instructors.</p>                    
                </div>
            </article>
            <article id="events">
                <div>
                    <h2>Events</h2>
                    <?php
                        $eventsData = json_decode(file_get_contents('../data/events.json'), true);

                        $events = new Events($eventsData);
                        $event = $events->getNextAvailableEvent();
                        
                        if (!$event->isEmpty()) {
                    ?>
                    
                        <p><?php echo $event->getFromDate(), ' at ' . $event->getStartTime(); ?></p>
                        <p>
                            <?php 
                            echo $event->getName() . ', with '; 
                            echo implode(', ', $event->getInstructors());
                            ?>
                        
                        </p>

                        <address>
                            <?php
                                echo $event->getDojo() . '<br />';
                                foreach ($event->getAddress() as $line) {
                                    echo $line . '<br />' . "\n";
                                }
                            ?>
                        </address>

                        <p>
                            <?php
                                echo $event->getDescription();
                            ?>
                        </p>                  

                        <?php 
                         if ($event->getUrl() !== null) {
                        ?>
                        <p>
                            <a href="<?php echo $event->getUrl(); ?>">
                                More info
                            </a>
                        </p>
                    <?php
                        }
                    } else { // event exists
                    ?>
                        
                        <p>Check the <a href="http://www.ukaonline.org.uk/UKA/index.php?option=com_content&view=article&id=106&Itemid=61">UKA site</a> for a list of events. </p>    
                    <?php    
                    }
                    ?>
                        
                </div>
            </article>
            <article id="resources">
                <div>
                    <h2>Resources</h2>
                    <ul>

                        <li>
                            <p><a href="http://www.ukaonline.org.uk/">United Kingdom Aikikai</a></p>
                        </li>
                        <li>
                            <p><a href='http://www.aikikai.or.jp/eng/'>Aikikai Foundation</a></p>
                        </li>
                        
                        <li>
                            <p><a href='http://www.britishbirankai.com/'>British Birankai</a></p>
                        </li>
                        
                        <li>
                            <p>
                                <a href="http://www.dmu-aikido.co.uk/">DMU Aikido Club</a>
                            </p>
                        </li>
                        <li>
                            <p>
                                <a href="http://www.airenjuku.org.uk/resources.htm">Airenjuku Aikido Club</a>
                            </p>
                        </li>
                        <li>
                            <p><a href="http://www.ninecircles.co.uk/">Nine Circles</a></p>
                        </li>
			<li>
			    </p><a href="http://www.martialartsuperstore.com/">Martial Art Superstore - Leicester</a></p>
			</li>
                    </ul>
                </div>
            </article>
<article id="location">
                <div>
                    <h2>Location</h2>
                    <p>Ei Mei Kan Aikido Dojo is located at the Faircharm Trading Estate.</p>
                  
                    <img class="large" src="img/EiMeiKan_Map_birdseye2.png" />
                    
                    <address>
                    Address: <br />
                    The Studio, <br />
                    Top Floor, <br />
                    Unit 36, <br />
                    Faircharm Trading Estate, <br />
                    Evelyn Drive, <br />
                    Leicester, <br />
                    LE3 2BU    
                    </address>
                    
                    <p>Coordinates: 52.618039,-1.148753</p> 
                    
                    <p>From De Montfort University</p>
                    <p>Walking:</p>
                    <ul>
                        <li>Take Upperton Road towards Narborough Road</li>
                        <li>On the first traffic lights, there is a footpath on the left</li>
                        <li>Follow the path towards Aylestone Meadows Park</li>
                        <li>When you see on your left a small bridge, you would have reached the Faircharm Trading Estate</li>
                        <li>Cross the bridge and at the end of the road turn left. The dojo is on Unit 36.</li>
                    </ul>



                    <p>By car:</p>
                    <ul>
                        <li>Take Upperton Road towards Narborough Road</li>
                        <li>On the Narborough Road (A5460) traffic lights turn left</li>
                        <li>At the traffic lights where KFC is on your right, turn left into Evesham Road</li>
                        <li>Take the first left into Heyworth Road</li>
                        <li>Take the first right into Evelyn Drive</li>
                        <li>Cross the bridge and at the end of the road turn left. The dojo is on Unit 36.</li>
                    </ul>


                    <p>Public transport:</p>
                    <ul>
                        <li>Bus Arriva/Midlands 104 towards Fosse Park / Asda</li>
                        <li>Leave on bus stop West End, adj Harlaxton Street (On Fosse Road South)</li>
                        <li>Turn right into Narborough Road</li>
                        <li>Turn left into Heyworth Road</li>
                        <li>Turn right into Evelyn Drive</li>
                        <li>Cross the bridge and at the end of the road turn left. The dojo is on Unit 36.</li>
                    </ul>



                    <p>From the motorway</p>
                    <ul>
                        <li>Follow signs for A5460</li>
                        <li>At the traffic lights where KFC is on your left, turn right into Evesham Road</li>
                        <li>Take the first left into Heyworth Road</li>
                        <li>Take the first right into Evelyn Drive</li>
                        <li>Cross the bridge and at the end of the road turn left. The dojo is on Unit 36.</li>
                    </ul>
                    
                    <div>
                        <img class="logo" src='img/UKA_logo.jpeg' />
                        <img class="logo" src='img/RenShinKan_Logo.jpg' />
                        
                    <div>
                    <div style='clear:both;' ></div>
                    
                </div>
            </article>            
        </section>
        <script src="//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script>
        <script>window.jQuery || document.write('<script src="js/vendor/jquery-1.9.1.min.js"><\/script>')</script>
        <script src="js/plugins.js"></script>
        <script src="js/aikido.js"></script>
<script type="text/javascript">

  var _gaq = _gaq || [];
  _gaq.push(['_setAccount', 'UA-41048220-1']);
  _gaq.push(['_trackPageview']);

  (function() {
    var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true;
    ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';
    var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
  })();

</script>        
    </body>
</html>
